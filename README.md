# ⚡ 실시간 15초 클릭 배틀 & TOP 15 토너먼트 결선 (Click Battle)

> 고등학생 40명을 대상으로 한 아케이드 감성의 실시간 15초 초고속 클릭 배틀 및 TOP 15 토너먼트 결선 웹 애플리케이션입니다.  
...
### 1. 📱 모바일 뷰 (학생용) - `/`
- **닉네임 및 아바타 선택**: 원하는 이모지 아바타와 닉네임(2~10자)을 정하고 대기실 입장 (비속어 필터링 및 랜덤 닉네임 생성 지원)
- **대기실 모드**: 새로고침해도 세션이 유지되며, 발표자가 게임을 시작할 때까지 대기 및 플레이 팁 안내
...
- **15초 광클릭 배틀**:
  ...
  - **막판 0.2초 버저비터 Flush**: 0초 도달 및 호스트 종료 신호 수신 즉시 로컬에 남아있던 마지막 0.2초 잔여 클릭 데이터를 강제 1회 전송하여 100% 공정한 결과 보장
- **🏆 TOP 15 결선 토너먼트 모드**:
  - 1라운드 15위 이내 진출자: 황금빛 챔피언십 배틀 화면 활성화
  - 비진출자: 실시간 **관전자(Spectator) 모드** (프로젝터 화면 관전 및 박수 응원 기능)
- **결과 화면**: 15초 종료 시 내 최종 클릭 수 및 최고 CPS 확인

### 2. 📺 프로젝터 뷰 (발표자용) - `/screen`
- **보안 비밀번호 잠금**: 발표자 화면 무단 진입 방지 (`SCREEN_PASSWORD` 환경변수 또는 기본값 `1234`)
...
- **[게임 시작] 버튼 & 스페이스바(SPACE) 단축키**: 3초 카운트다운 후 15초 게임 일괄 시작 브로드캐스트 발송
- **게임 일괄 중지 (ESC)**: 비상 시 언제든 게임을 즉시 멈추고 대기실로 리셋
- **실시간 1~40등 2단 컬럼 순위표**:
  - **좌측 컬럼**: 1등 ~ 20등 (Champion Bracket)
  - **우측 컬럼**: 21등 ~ 40등 (Challenger Bracket)
  - 1, 2, 3위 황금/은/동 뱃지 및 점수 비율 게이지, 실시간 CPS 표시
- **종료 시상식 및 🏆 TOP 15 결선 배틀 연계**:
  - 15초 종료 휘슬과 함께 1, 2, 3위 입체 포디엄(시상대) 연출 및 `canvas-confetti` 축하 폭죽 발사
  - **[🏆 TOP 15 결선 배틀 시작! (15s FINAL)]** 원클릭으로 상위 15명을 선발하여 결승전 즉시 진행
...
## 🔒 발표자 화면(/screen) 비밀번호 보안 설정

학생들이 임의로 프로젝터/발표자 화면에 접근하는 것을 방지하기 위해 비밀번호 잠금 기능이 적용되어 있습니다.
- **환경 변수**: `SCREEN_PASSWORD`
- **로컬/Vercel 환경 변수 등록 예시**:
  ```env
  SCREEN_PASSWORD=나만의비밀번호
  ```
- *미설정 시 기본 비밀번호는 `1234`로 동작합니다.*
- 비밀번호를 한 번 입력하여 인증되면 브라우저 세션에 안전하게 유지되며, 헤더의 🔒(자물쇠) 버튼을 눌러 언제든지 다시 잠글 수 있습니다.
